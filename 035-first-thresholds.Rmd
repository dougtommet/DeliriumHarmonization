...
goo3 <- fscores %>%
  select(recordid, f_se.dich.drs, f_se.dich.mdas, f_se.dich.cam) %>%
  gather(test, se.dich, 2:4) %>%
  separate(test, sep="f_se.dich.", into = c("foo1", "test")) %>%
  select(-foo1) 

goo4 <- fscores %>%
  select(recordid, f_se.poly.drs, f_se.poly.mdas, f_se.poly.cam) %>%
  gather(test, se.poly, 2:4) %>%
  separate(test, sep="f_se.poly.", into = c("foo1", "test")) %>%
  select(-foo1) 

goo5 <- fscores %>%
  select(recordid, f.dich.drs, f.dich.mdas, f.dich.cam) %>%
...
testinfo <- drs.info.poly %>%
  ...
  left_join(cam.info.poly %>%
              select(theta, sem.sens) %>%
              rename(camsem = sem.sens), by="theta") %>%
  gather(test, sem, 2:4)
...
testinfo %>%
  ggplot(aes(theta, sem, group=test, color=test)) +
    ...
    scale_color_brewer(palette = "Dark2", 
                     name = "Delirium Instrument",
                     breaks = c("drssem", "mdassem", "camsem"),
                     labels = c("DRS", "MDAS", "CAM-S")) + 
