...
foo.mdas<-  mdas.par.poly %>%
  ...
  arrange(itemnum, B) %>%
  rename(A = A.mdas) %>%
  mutate(gr.order = recode(itemnum, "10=1; 5=2; 3=4; 4=5; 2=7; 6=8; 1=11; 7=12; 9=14; 8=13;"),
...
foo.cam<-  cam.par.poly %>%
  ...
  arrange(itemnum, B) %>%
  rename(A = A.cam) %>%
  mutate(gr.order = recode(itemnum, "10=1; 2=2; 6=4; 1=6; 5=7; 3=8; 4=11; 7=12; 8=15; 9=16;"),
         gr.order = gr.order+.5,
         test = "CAM",
         itemnum = itemnum+30) 
...
plot_thresholds <-  foo.drs %>%
  ...
  ggplot(aes(x = est, y = gr.order, group=itemnum, color = test)) +
    ...
    annotate("text", x = pos1, y = c3.loc, hjust=0, label = "CAM-S 3", 
             size = labsize) +
    annotate("text", x = pos1, y = c4.loc, hjust=0, label = "CAM-S 4", 
             size = labsize) +
    annotate("text", x = pos1, y = c5.loc, hjust=0, label = "CAM-S 5", 
             size = labsize) +
    annotate("text", x = pos1, y = c6.loc, hjust=0, label = "CAM-S 6", 
             size = labsize) +
    annotate("text", x = pos1, y = c7.loc, hjust=0, label = "CAM-S 7", 
             size = labsize) +
    annotate("text", x = pos1, y = c8.loc, hjust=0, label = "CAM-S 8a", 
             size = labsize) +
    annotate("text", x = pos1, y = c9.loc, hjust=0, label = "CAM-S 8d", 
             size = labsize) +
    annotate("text", x = pos1, y = c10.loc, hjust=0, label = "CAM-S 9", 
             size = labsize) +
