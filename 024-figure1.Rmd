...
## Data for rug plots
## (using geom_point instead of geom_rug so that the instruments can be on their own line.)
fscores.drs <- readModels("cfa-drs-3-1.out", quiet = TRUE)[["savedata"]] %>%
  setNames(tolower(names(.))) %>%
  select(recordid, f) %>%
  rename(DRS = f)

fscores.mdas <- readModels("cfa-mdas-3-1.out", quiet = TRUE)[["savedata"]] %>%
  setNames(tolower(names(.))) %>%
  select(recordid, f) %>%
  rename(MDAS = f)

fscores.cam <- readModels("cfa-cam-3-1.out", quiet = TRUE)[["savedata"]] %>%
  setNames(tolower(names(.))) %>%
  select(recordid, f) %>%
  rename(CAM = f)
