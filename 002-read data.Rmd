...
inhosp <- read_dta(str_c(dataprocessing, inhosp.filename))
inhosp <- inhosp %>%
  arrange(studyid, cvpg3a)
...
patientinterview <- read_dta(str_c(dataprocessing, patientinterview.filename)) %>%
  filter(timefr==0) %>%
...
medical <- read_dta(str_c(dataprocessing, medical.filename)) %>%
