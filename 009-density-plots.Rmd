...
fscores.drs <- readModels("cfa-drs-2-1-2pl.out")[["savedata"]] %>%
  setNames(tolower(names(.))) %>%
  select(recordid, f) %>%
  rename(f.drs = f)

fscores.mdas <- readModels("cfa-mdas-link-1.out")[["savedata"]] %>%
  setNames(tolower(names(.))) %>%
  select(recordid, f) %>%
  rename(f.mdas = f)

fscores.cam <- readModels("cfa-cam-link-1.out")[["savedata"]] %>%
  setNames(tolower(names(.))) %>%
  select(recordid, f) %>%
  rename(f.cam = f)
