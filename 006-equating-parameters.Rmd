...
```{r}
# drs.par.dich <- readModels("cfa-drs-2-1-2pl.out")$parameters$unstandardized
# 
...
#       
# 
# mdas.par.dich <- readModels("cfa-mdas-2-1-2pl.out")$parameters$unstandardized
# 
...
# 
# 
# cam.par.dich <- readModels("cfa-cam-2-1-2pl.out")$parameters$unstandardized
# 
...
getIRTparam <- function(filename, instrument, itemnum) {
  par.dich <- readModels(filename)$parameters$unstandardized
...
cam.par.dich <- getIRTparam("cfa-cam-2-1-2pl.out", "cam", c(22, 10, 6, 25, 9, 26, 2, 7, 8, 1))
...
```
```{r} 
item.parameter.table2 <- function(param.table, instrument.name){
  foo <- param.table %>%
    select(-starts_with("C"), -itemnum) 
  
  colnames(foo) <- c("item", "A", "B")
  
  foo %>%
    arrange(desc(A)) %>%
    kable(digits=2,
          col.names = c("Item", "A", "B"),
          caption = paste("Item parameters for", instrument.name)) %>%
...
### DRS item parameters
```{r}
item.parameter.table2(drs.par.dich, "DRS")
```

### MDAS item parameters
```{r}
item.parameter.table2(mdas.par.dich, "MDAS")
```

### CAM-S item parameters
```{r}
item.parameter.table2(cam.par.dich, "CAM")
```
