...
```{r}
drs.par <- readModels("cfa-drs-2.out")$parameters$unstandardized
...
drs.par <- drs.par %>%
  ...
  drs.par <- drs.par[1:26,1:3]
  drs.par$est <- as.numeric(drs.par$est)
...
mdas.par <- readModels("cfa-mdas-2.out")$parameters$unstandardized
...
mdas.par <- mdas.par %>%
    ...
    select(parameter, item, est) %>%
    mutate_all(funs(tolower))
  mdas.par <- mdas.par[1:20,1:3]
  mdas.par$est <- as.numeric(mdas.par$est)
...
mdas.par$itemnum <- c(14, 9, 11, 16, 17, 18, 2, 3, 21, 1)


cam.par <- readModels("cfa-cam-2.out")$parameters$unstandardized
...
cam.par <- cam.par %>%
    ...
    select(parameter, item, est) %>%
    mutate_all(funs(tolower))
  cam.par <- cam.par[1:20,1:3]
  cam.par$est <- as.numeric(cam.par$est)
...
cam.par$itemnum <- c(22, 10, 24, 25, 9, 26, 2, 7, 8, 1)
...
```{r}
par.drs.mdas <- full_join(drs.par, mdas.par, by = "itemnum") %>%
...
```{r}
par.drs.cam <- full_join(drs.par, cam.par, by = "itemnum") %>%
